...
precise value $1/2$ (with a slow upward drift, Table~\ref{tab:beta}) is left as the
central open problem of the project.

\begin{conjecture}[Half-power singular law]
As $X\to\infty$,
\[
  \frac{\sum_{N\le X}\log\!\big(R_2/R_1\big)\,\log\sing(N)}
       {\sum_{N\le X}\log^2\sing(N)}\ \longrightarrow\ -\tfrac12 ,
\]
i.e.\ $R_2(N)/R_1(N)\asymp\sing(N)^{-1/2}$ in logarithmic mean.
\end{conjecture}
