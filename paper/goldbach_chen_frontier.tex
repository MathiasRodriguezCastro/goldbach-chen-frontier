...
\]
so that $\beta_2\to1$ with a deficit of order $1/\log\log N$: the apparent ``half power'' near $X\sim10^{6}$ is a finite-range artefact, and $R_2$ asymptotically inherits the \emph{full} Goldbach singular series. Extending to layers $\OmegaF(q)=k$, the singular exponent $\beta_k$ decreases through $1,\tfrac12,\dots$ and turns negative for $k\ge3$, so the singular series enhances low-complexity and suppresses high-complexity channels. On the theoretical side we reduce the law $\beta_2\to1$ to standard analytic inputs: an
appendix gives an unconditional singular-series identity, a first-moment reduction to the
equidistribution of primes in progressions (Siegel--Walfisz / Bombieri--Vinogradov), and a
variance bound from the classical Vinogradov minor-arc estimate, yielding $\beta_2(N)\to1$
for \emph{almost all} $N$ conditional only on inputs of Bombieri--Vinogradov / Chen strength
rather than on a pointwise Hardy--Littlewood asymptotic. We complement this with a broad
suite of reproducible reframings, all of which turn out to be governed by the same singular
series: diagnostics (Chen surplus, fragility, factor geometry) and weak-continuity of the
$q/N$ measures; mixed-integer minimum-variation selection and a unimodal Chen-rescue
threshold; a residual balance exponent $a_{\rm Res}(N)$ in the spirit of the $(1+a)$
interpolation; an arithmetic ground-state energy whose Goldbach order melts at a temperature
$\beta^\star$ anticorrelated with $\sing(N)$; the adversarial robustness of the Goldbach
graph (random-robust but structure-fragile); a market reading in which the Chen substitute is
fair-weather liquidity; a control policy quantifying the value of arithmetic information; an
empirical circle method whose comet spectrum is the major arcs and whose information content
is $99.9\%$ compressible by local arithmetic; the discrete geometry of the surface $p+rs=N$;
and a primorial-base and carry-geometry analysis exhibiting a digital signature of primality.
The goal is not a proof of Goldbach, but reproducible evidence and new regularities at the
boundary between additive number theory, computational experimentation, and discrete
optimization---unified by the Hardy--Littlewood singular series.
\end{abstract}
...
\item \textbf{A reduction towards a theorem (Appendix~\ref{app:rigorous}).} The law $\beta_2\to1$ is reduced to standard analytic number theory: an unconditional singular-series identity (Lemma~\ref{lem:sing}), a first-moment reduction to Siegel--Walfisz / Bombieri--Vinogradov, and a variance bound from Vaughan's minor-arc estimate, giving $\beta_2(N)\to1$ for almost all $N$ conditional only on Bombieri--Vinogradov / Chen-strength inputs.
\item \textbf{A dozen reframings, one singular series (\S\ref{sec:energy-emp}--\ref{sec:bases-emp}).} Goldbach is recast as a thermodynamic ground state, an adversarial network, a liquidity market, a control problem, an empirical circle method, an information channel, a discrete surface, and a carry geometry. In each, the governing quantity is the Hardy--Littlewood singular series $\sing(N)$: it sets the melting temperature $\beta^\star$, the modular fragility, the substitute spread, the value of information, the comet spectrum (which is $99.9\%$ compressible), the concentration of Chen solutions toward Goldbach, and the carry signature of primality.
\end{enumerate}
...
the singular series. Thus the same local arithmetic that the singular series encodes is
directly actionable: a controller that ``knows'' $N$ modulo small primes clears the
Goldbach market roughly twice as efficiently, quantifying the value of arithmetic
information for additive prime decompositions.
...
\]
The comet is thus $99.9\%$ compressible by local arithmetic. The remaining $0.11\%$ is the
genuine Hardy--Littlewood fluctuation: its power spectrum is \emph{flat}---white noise, in
contrast to the sharply peaked spectrum of $R_1$ itself (\S\ref{sec:spectral-emp})---with
lag-$1$ autocorrelation $-0.04$ and standard deviation $2.7\%$, a few times the Poisson
floor $\langle R_1\rangle^{-1/2}=0.9\%$. In information terms, the Goldbach comet is almost
entirely a singular-series codeword; its incompressible core carries no further arithmetic
signal and is the structureless randomness of the primes. There is no hidden order below
the singular series---a clean negative answer to whether anomalous $N$ are
``informationally special''.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{22_informacion.png}
\caption{Information budget of the comet. Left: the variance of $\log R_1$ splits into a
smooth trend ($81\%$), the singular series ($18.8\%$), and an irreducible residual
($0.11\%$). Right: the residual's power spectrum is flat---white noise, with none of the
arithmetic peaks of Fig.~\ref{fig:spectral}.}
\label{fig:info}
\end{figure}
