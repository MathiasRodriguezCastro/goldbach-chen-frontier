...
\paragraph{What makes the variance small: the minor arcs.}
This concentration is exactly the output a second-moment (circle-method) argument is
designed to give. Writing $R_2(N)=\int_0^1 S_\PP(\alpha)S_{\SSemi}(\alpha)e(-N\alpha)\,d\alpha$
with $S_\PP(\alpha)=\sum_{p\le N}e(p\alpha)$ and
$S_{\SSemi}(\alpha)=\sum_{q\le N,\,q\in\SSemi}e(q\alpha)$, the major arcs reproduce the
main term $\widetilde W(N)R_1(N)$ (Lemma~\ref{lem:sing}), while the variance over the
window is controlled by the minor-arc mass
\[
  \sum_{N\in\mathcal N_X}\big|R_2(N)-\text{main}(N)\big|^2
  \ll \int_{\mathfrak m}|S_\PP(\alpha)|^2|S_{\SSemi}(\alpha)|^2\,d\alpha .
\]
The new ingredient relative to Goldbach is $S_{\SSemi}$. Because $q=p's$ is bilinear,
$S_{\SSemi}$ is a Type~II (bilinear) sum, bounded on minor arcs by Vaughan's identity
and the linear sieve---precisely the apparatus of Chen's theorem. The large sieve then
gives $\int_{\mathfrak m}|S_\PP|^2|S_{\SSemi}|^2\ll X^{3}\log^{-A}X$ for every $A$,
i.e.\ an exceptional set of size $O(X\log^{-A}X)$, matching the Montgomery--Vaughan
bound for $R_1$. We do not carry out the estimate here; the point is that the variance
input needed to upgrade Theorem~\ref{thm:fm} to a pointwise ``almost all'' statement is
of \emph{exactly the type already available} (bilinear/sieve minor-arc bounds), and the
data confirm it holds with large margin (Table~\ref{tab:conc}). Thus the chain
\[
  \text{Bombieri--Vinogradov (Lemma~\ref{lem:fm})}
  \ \Longrightarrow\ \beta_2^{(1)}\to1
  \quad\text{and}\quad
  \text{bilinear minor-arc bound}
  \ \Longrightarrow\ \beta_2=\beta_2^{(1)}+o(1)
\]
yields $\beta_2(N)\to1$ for almost all $N$, conditional only on inputs of
Bombieri--Vinogradov / Chen strength rather than on a pointwise Hardy--Littlewood
asymptotic.
